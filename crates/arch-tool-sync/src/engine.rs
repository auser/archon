...
pub fn run_sync(
    ...
    if !manifest_path.exists() {
        anyhow::bail!(
            "no sync-manifest.yaml found in {}. Run `arch-tool init` first.",
            repo_root.display()
        );
    }
...
fn sync_section_managed(
    ...
    let dest_content = match std::fs::read_to_string(&dest_path) {
        ...
        Err(_) => {
            return SyncFileResult {
                path: rel_path.to_string(),
                action: SyncAction::Error(format!("{rel_path} not found — create it with `arch-tool init` first")),
            }
        }
    };
    ...
    match sections::replace_managed_section(&dest_content, new_section_content) {
        ...
        None => SyncFileResult {
            ...
            action: SyncAction::Error(format!(
                "{rel_path} has no ARCH-TOOL:MANAGED markers — add them first"
            )),
        },
    }
}
