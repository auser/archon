...
pub struct SyncEntry {
    ...
    pub path: String,
    /// How arch-tool manages this file.
...
/// File ownership model — controls how `arch-tool sync` handles each file.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
#[serde(rename_all = "kebab-case")]
pub enum Ownership {
    /// Entire file is owned by arch-tool and overwritten on sync.
    FullyManaged,
    /// Only content between ARCH-TOOL:MANAGED markers is replaced.
    SectionManaged,
    /// arch-tool never touches this file.
    Unmanaged,
}
