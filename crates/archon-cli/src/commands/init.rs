...
use archon_ai::context::{FillContext, ProjectContext};
use archon_ai::doc_selection;
use archon_ai::fill;
use archon_core::paths;
use archon_core::paths::REPO_META_FILENAME;
use archon_core::profile::Profile;
use archon_core::standards_version::StandardsVersion;
use archon_templates::renderer;
...
pub fn run_in(root: &Path, args: InitArgs) -> Result<()> {
    ...
    // Execute
    output::print_header(&format!("archon init: {repo_name} (profile: {profile})"));
    println!();
