...
use arch_tool_adr::numbering;
use arch_tool_ai::backend;
use arch_tool_ai::decide::{self, DecisionContext};
...
pub fn run(args: DecideArgs) -> Result<()> {
    let arch_root = paths::resolve_arch_root(args.arch_root.as_deref())?
        .ok_or_else(|| {
            anyhow::anyhow!(
                "architecture repo not found. Use --arch-root or set ARCH_TOOL_ROOT.\n  \
    ...
    output::print_header(&format!("arch-tool decide: {title}"));
    println!();
    ...
    if args.dry_run {
        ...
        if let Some(ref snippet) = draft.policy_snippet {
            let suggestion_dir = arch_root.join(".arch-tool/suggestions");
            ...
            eprintln!(
                "  {} policy suggestion saved to .arch-tool/suggestions/{}-{slug}.yaml",
                "✓".green(),
