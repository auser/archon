...
pub fn run(args: BootstrapArgs) -> Result<()> {
    ...
    output::print_header(&format!("arch-tool bootstrap: {name}"));
    println!();
    ...
    if args.dry_run {
        ...
        eprintln!(
            "    6. Set ARCH_TOOL_ROOT={} in your shell",
            target.display()
        );
    }
...
- **Machine-readable policies** (`policies/`) — rules that `arch-tool verify` enforces
- **Sync templates** (`templates/`) — content pushed to downstream repos via `arch-tool sync`
...
```
{name}               arch-tool                   downstream repos
...
**arch-tool never writes to this repo.** The flow is strictly one-way: architecture → tool → downstream repos.

## Quick start

```bash
# Add a new architecture decision
arch-tool adr new --title "Require API documentation"

# Test a policy change against all repos
for repo in ../hologram ../hologram-ai; do
  (cd "$repo" && arch-tool verify --arch-root ../{name})
done

# Sync updated templates to downstream repos
for repo in ../hologram ../hologram-ai; do
  (cd "$repo" && arch-tool sync --arch-root ../{name})
done
```
...
- **tool**: development and governance tools (arch-tool)
...
- `arch-tool verify` checks that role declarations match actual structure
...
- `arch-tool graph` visualizes the contract dependency graph
...
- `arch-tool graph` can detect circular dependencies and orphaned contracts
...
## Consequences
- `arch-tool verify` can validate crate naming in `Cargo.toml`
...
- `arch-tool verify` warns when a repo is behind the current standards version
...
- `arch-tool status` shows each repo's version relative to current
...
- **fully-managed**: arch-tool owns the entire file. Overwritten from source on every sync.
- **section-managed**: arch-tool owns content between `<!-- ARCH-TOOL:MANAGED:BEGIN -->` and `<!-- ARCH-TOOL:MANAGED:END -->` markers. Handwritten content outside markers is preserved.
- **unmanaged**: arch-tool never modifies the file.
...
## Consequences
- `arch-tool sync` only updates files that have changed (SHA-256 hash tracking)
...
- `arch-tool sync` keeps the managed sections current
- `arch-tool verify` checks that `AGENTS.md` exists (STR-002)
...
- Run `arch-tool verify` to check conformance
- Run `arch-tool sync` to pull latest managed content from the architecture repo
...
This document is managed by `arch-tool sync` from the architecture repository. Do not edit it directly in downstream repos.
...
| library | Shared libraries and SDKs | hologram-sdk |
| service | Deployed services | hologram-website |
| tool | Development and governance | arch-tool |
...
The ecosystem uses date-based standards versioning (`YYYY.MM`). Each repo declares which standards version it follows. `arch-tool verify` checks conformance.

## Governance

Architecture decisions are recorded as ADRs in the architecture repository. Policy rules encode those decisions as machine-readable checks. `arch-tool` enforces them across all repos.
"#
            .to_string(),
        },
        // ── Standards ───────────────────────────────────────────────
...
# arch-tool graph reads this to build the contract dependency map.
...
# This file is generated by arch-tool graph and should not be edited manually.
# Run: arch-tool graph --repos-dir .. --format yaml > ecosystem/contract-graph.yaml
...
| `policies/` | Machine-readable rules enforced by `arch-tool verify` |
| `templates/` | Content synced to downstream repos via `arch-tool sync` |
...
3. **Templates affect all downstream repos** — changes to `templates/` propagate on `arch-tool sync`. Be deliberate.
...
This section is managed by arch-tool. Do not edit manually.
...
The enforcement tool is `arch-tool` (a separate repo).

## Key directories

- `specs/adrs/` — Architecture Decision Records
- `policies/` — YAML rules loaded by `arch-tool verify`
